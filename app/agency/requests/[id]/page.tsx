...
export default function AgencyInquiryDetailPage() {
  ...
  if (error || !inquiry) {
    return (
      <div>
        <AgencyPageHeader title="Inquiry" backHref="/agency/requests" />
        <p className="text-[14px] text-ink-muted">{error || "Inquiry not found"}</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-6">
      <AgencyPageHeader
        ...
        subtitle={`From ${inquiryPersonName(inquiry.employer)}`}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          <AgencyCard className="p-5">
            <h2 className="mb-3 text-[15px] font-bold text-ink">Employer</h2>
              ...
              <div className="mt-3 space-y-1.5 border-t border-gray-50 pt-3 text-[12px] text-ink-muted">
                ...
                {inquiry.employer.createdAt && (
                  <p className="flex items-center gap-1.5">
                    <CalendarClock className="h-3.5 w-3.5" /> Member since {new Date(inquiry.employer.createdAt).toLocaleDateString("en-RW", { month: "short", year: "numeric" })}
                  </p>
                )}
              </div>
            )}
            <div className="mt-4">
              <p className="mb-1.5 flex items-center gap-1.5 text-[13px] font-semibold text-ink"><MessageSquare className="h-4 w-4 text-ink-muted" /> Their note</p>
          ...
          {inquiry.workerOfInterest && (
            <AgencyCard className="p-5">
              <h2 className="mb-3 text-[15px] font-bold text-ink">Worker of interest</h2>
            ...
            <AgencyCard className="p-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[13px] text-ink">
                  Waiting for <span className="font-bold">{inquiryPersonName(inquiry.handoverWorker)}</span> to accept the placement.
                </p>
              </div>
              <button
                onClick={() => act("handover/cancel", undefined, "Offer withdrawn")}
                disabled={busy !== null}
              ...
              >
                Cancel offer &amp; choose another worker
              </button>
            </AgencyCard>
          )}
          {inquiry.status === "CONVERTED" && (
            <AgencyCard className="flex items-center gap-3 p-4">
              ...
              <p className="text-[13px] text-ink">
                Placed with <span className="font-bold">{inquiryPersonName(inquiry.handoverWorker)}</span>. The employer and worker now have a booking.
              </p>
            </AgencyCard>
          )}
          {inquiry.status === "DECLINED" && (
            <AgencyCard className="flex items-center gap-3 p-4">
              ...
              <p className="text-[13px] text-ink">You declined this inquiry.</p>
            </AgencyCard>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4">
          {isPending && (
            <AgencyCard className="p-5">
              <h2 className="text-[15px] font-bold text-ink">Respond</h2>
              <p className="mt-0.5 text-[12px] text-ink-muted">Accept to open a conversation with the employer, or decline.</p>
              <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                <button
                  onClick={() => act("accept", undefined, "Conversation opened")}
                  disabled={busy !== null}
                ...
                >
                  <MessageSquare className="h-5 w-5" /> Accept to talk
                </button>
                <button
                  onClick={() => act("decline", undefined, "Inquiry declined")}
                  disabled={busy !== null}
          ...
          {(isTalking || inquiry.status === "HANDED_OVER" || inquiry.status === "CONVERTED") && (
            <AgencyCard className="flex flex-col p-5">
              <h2 className="mb-3 text-[15px] font-bold text-ink">Conversation</h2>
          ...
          {isTalking && (
            <AgencyCard className="p-5">
              <h2 className="flex items-center gap-2 text-[15px] font-bold text-ink"><Users className="h-4 w-4 text-brand" /> Hand over a worker</h2>
              <p className="mt-0.5 text-[12px] text-ink-muted">When you&apos;ve agreed, propose a worker. They&apos;ll confirm, then a booking is created.</p>
              <select
                value={handoverId}
              ...
              <button
                onClick={() => handoverId && act("handover", { workerId: handoverId }, "Sent to the worker for confirmation")}
              ...
              >
                <Briefcase className="h-5 w-5" /> Hand over worker
              </button>
            </AgencyCard>
          )}
        </div>
